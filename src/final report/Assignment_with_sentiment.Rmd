...
```{r}
language_short <- list("Eng", "Heb", "Ara", "Dut","Fre", "Ger", "Ita", "Spa", "Chi", "Rus", "Per")

par(mfrow = c(4, 3))

for (item in language_short) {
  df <- filter(content_filtered, language_short == item)

  scores <- df$sentiment_score
  
  hist(scores, names.arg = 1:length(scores), xlab = "Line", ylab = "Sentiment Score", main = item)
